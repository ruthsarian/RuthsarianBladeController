...
FUSES = {
  ...
  .BODCFG     = LVL_BODLEVEL2_gc | BOD_SAMPFREQ_125HZ_gc 
              | BOD_ACTIVE_SAMPLED_gc | BOD_SLEEP_SAMPLED_gc,         // FUSE BYTE 1: 2.6V BOD threshold, sampled/sampled @ 125Hz
...
void sleep_handler(void) {
  ...
  if ((blade.state & 0xF0) == BLADE_STATE_OFF) {
    ...
    // has it been off for more than X seconds?
      ...
      // did the switch config change while the blade was asleep?
      //
      // this is possible because in deep sleep such little power is used that a pulled blade's controller
      // will continue to function for a few minutes, during which a user may have changed switch settings
      // and reinserted the blade. 
      if (switch_config != get_switch_config()) {

        // reload state from eeprom
        eeprom_load_state();

        // reload switch settings
        switch_setup();
      }
